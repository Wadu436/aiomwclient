...
class Image(aiomwclient.page.Page):
    ...
    def imageusage(
        ...
        """
        prefix = aiomwclient.listing.List.get_prefix("iu", generator)
        kwargs = dict(
    ...
    def duplicatefiles(self, limit=None):
        ...
        """
        return aiomwclient.listing.PageProperty(
            self, "duplicatefiles", "df", dflimit=limit
        )
    ...
    async def download(self, destination=None):
        ...
        if destination is not None:
            res = await self.site.connection.get(url, stream=True)
            for chunk in res.iter_content(1024):
                destination.write(chunk)
        else:
            return await self.site.connection.get(url).content
