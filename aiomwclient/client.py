...
import aiomwclient.errors as errors
import aiomwclient.listing as listing
from aiomwclient.sleep import Sleepers
from aiomwclient.util import parse_timestamp, read_in_chunks

__version__ = "0.0.1"

log = logging.getLogger(__name__)

USER_AGENT = "mwclient/{} ({})".format(
    __version__, "https://github.com/mwclient/mwclient"
)
...
class Site(object):
    """A MediaWiki site identified by its hostname.
        ...
        >>> import mwclient
        >>> site = aiomwclient.Site('en.wikipedia.org')
    ...
    Examples:
        ...
        >>> site = aiomwclient.Site('vim.wikia.com', path='/')
        >>> site = aiomwclient.Site('sourceforge.net', path='/apps/mediawiki/mwclient/')
    ...
    async def __init__(
        ...
        if isinstance(httpauth, (list, tuple)):
            ...
            auth = aiohttp.BasicAuth(*httpauth)

        elif httpauth is None or isinstance(httpauth, (aiohttp.BasicAuth,)):
            auth = httpauth
        else:
            raise RuntimeError(
        ...
        self.connection = aiohttp.ClientSession(headers=headers, auth=auth)

        if client_certificate:
            # self.connection.cert = client_certificate
            if isinstance(client_certificate, (tuple, list)):
    ...
    async def site_init(self):
        ...
        if self.initialized:
            info = await self.get("query", meta="userinfo", uiprop="groups|rights")
        ...
        self.namespaces = {
            ...
            for namespace in iter(meta["query"]["namespaces"].values())
        }
    ...
    async def close(self):
        ...
        Should be called once you're done with the object to properly close the HTTP connection."""
    ...
    def version_tuple_from_generator(string, prefix="MediaWiki "):
        ...
        if len(version_tuple) < 2:
            raise errors.MediaWikiVersionError(
                "Unknown MediaWiki {}".format(".".join(version))
            )
    ...
    async def get(self, action, *args, **kwargs):
        ...
        """
        return await self.api(action, "GET", *args, **kwargs)
    ...
    async def post(self, action, *args, **kwargs):
        ...
        Returns:
            The raw response from the API call, as a dictionary.
        """
        return await self.api(action, "POST", *args, **kwargs)

    async def api(self, action, http_method="POST", *args, **kwargs):
        ...
        while True:
            info = await self.raw_api(action, http_method, **kwargs)
            if not info:
                info = {}
            if await self.handle_api_result(info, sleeper=sleeper):
                return info

    async def handle_api_result(self, info, kwargs=None, sleeper=None):
        if sleeper is None:
            sleeper = self.sleepers.make()
        ...
        if "error" in info:
            ...
            if info["error"].get(
                "code"
            ) == u"mwoauth-invalid-authorization" and "Nonce already used" in info[
                "error"
            ].get(
                "info"
            ):
            ...
            if "*" in info["error"]:
                raise errors.APIError(
                    info["error"]["code"], info["error"]["info"], info["error"]["*"]
                )
    ...
    def _query_string(*args, **kwargs):
        kwargs.update(args)
        qs1 = [
            (k, v) for k, v in iter(kwargs.items()) if k not in {"wpEditToken", "token"}
        ]
        qs2 = [(k, v) for k, v in iter(kwargs.items()) if k in {"wpEditToken", "token"}]
        return OrderedDict(qs1 + qs2)

    async def raw_call(
        self, script, data, files=None, retry_on_error=True, http_method="POST"
    ):
        """
        ...
        url = "{scheme}://{host}{path}{script}{ext}".format(
            scheme=scheme, host=host, path=self.path, script=script, ext=self.ext
        )
        ...
        while True:
            try:
                ...
                async with self.connection.request(
                    method=http_method, url=url, ssl=self.ssl, **args
                ) as stream:
                    if stream.headers.get("x-database-lag"):
                        ...
                        await sleeper.sleep(wait_time)
                    elif stream.status == 200:
                        return await stream.text()
                    elif stream.status < 500 or stream.status_code > 599:
                        ...
                        log.warning(
                            ...
                            "Retrying in a moment.".format(
                                status=stream.status, text=await stream.text()
                            )
                        )
                        await sleeper.sleep()
    ...
    async def raw_api(self, action, http_method="POST", *args, **kwargs):
        ...
        res = await self.raw_call(
            "api", data, retry_on_error=retry_on_error, http_method=http_method
        )
    ...
    async def raw_index(self, action, http_method="POST", *args, **kwargs):
        """Sends a call to index.php rather than the API."""
        kwargs["action"] = action
        kwargs["maxlag"] = self.max_lag
        data = self._query_string(*args, **kwargs)
        return await self.raw_call("index", data, http_method=http_method)
    ...
    # Actions
    async def email(self, user, text, subject, cc=False):
        """
        Send email to a specified user on the wiki.

            >>> try:
            ...     site.email('SomeUser', 'Some message', 'Some subject')
            ... except aiomwclient.errors.NoSpecifiedEmail:
    ...
    async def login(self, username=None, password=None, cookies=None, domain=None):
        """
        ...
        if cookies:
            self.connection.cookie_jar.update_cookies(cookies)
        ...
        if self.credentials:
            ...
            while True:
                ...
                if login["login"]["result"] == "Success":
                    ...
                    await sleeper.sleep(int(login["login"].get("wait", 5)))
                else:
                    raise errors.LoginError(
                        self, login["login"]["result"], login["login"]["reason"]
                    )

        await self.site_init()

    async def get_token(self, type, force=False, title=None):
        ...
        if self.tokens.get(type, "0") == "0" or force:
            ...
            if self.version is None or self.version[:2] >= (1, 24):
                ...
                info = await self.raw_api("query", "GET", meta="tokens", type=type)

                await self.handle_api_result(info)
                ...
                if title is None:
                    ...
                    title = "Test"
                info = await self.post("query", titles=title, prop="info", intoken=type)
                for i in iter(info["query"]["pages"].values()):
                    if i["title"] == title:
    ...
    async def upload(
        ...
        if file_size is not None:
            ...
            warnings.warn(
                "file_size is deprecated since mwclient 0.7", DeprecationWarning
            )
        ...
        if file is not None:
            ...
            if self.version[:2] >= (1, 20) and content_size > self.chunk_size:
                return await self.chunk_upload(file, filename, ignore, comment, text)
        ...
        predata = {
            ...
            "comment": comment,
            "text": text,
            "token": await image.get_token("edit"),
        }
        ...
        sleeper = self.sleepers.make()
        while True:
            data = await self.raw_call("api", postdata, files)
            info = json.loads(data)
            if not info:
                info = {}
            if await self.handle_api_result(info, kwargs=predata, sleeper=sleeper):
    ...
    async def chunk_upload(self, file, filename, ignorewarnings, comment, text):
        ...
        for chunk in read_in_chunks(file, self.chunk_size):
            while True:
                data = await self.raw_call("api", params, files={"chunk": chunk})
                info = json.loads(data)
                if await self.handle_api_result(info, kwargs=params, sleeper=sleeper):
                    response = info.get("upload", {})
                    break
        ...
        params["text"] = text
        return await self.post("upload", **params)
    ...
    async def parse(
        ...
        result = await self.post("parse", **kwargs)
        return result["parse"]
    ...
    def allpages(
        ...
        return listing.List.get_list(generator)(
            self, "allpages", "ap", limit=limit, return_values="title", **kwargs
        )
    ...
    def alllinks(
        ...
        if unique:
            kwargs[pfx + "unique"] = "1"
        return listing.List.get_list(generator)(
            self, "alllinks", "al", limit=limit, return_values="title", **kwargs
        )
    ...
    def allcategories(
        ...
        kwargs = dict(
            listing.List.generate_kwargs(
                pfx, ("from", start), ("to", end), prefix=prefix, dir=dir
            )
        )
        return listing.List.get_list(generator)(
            self, "allcategories", "ac", limit=limit, **kwargs
        )
    ...
    def blocks(
        ...
        Returns:
            aiomwclient.listings.List: Generator yielding dicts, each dict containing:
        ...
        kwargs = dict(
            listing.List.generate_kwargs(
                "bk", start=start, end=end, dir=dir, ids=ids, users=users, prop=prop
            )
        )
    ...
    def deletedrevisions(
        ...
        kwargs = dict(
            listing.List.generate_kwargs(
                "dr", start=start, end=end, dir=dir, namespace=namespace, prop=prop
            )
        )
        return listing.List(self, "deletedrevs", "dr", limit=limit, **kwargs)

    def exturlusage(
        self, query, prop=None, protocol="http", namespace=None, limit=None
    ):
        ...
        kwargs = dict(
            listing.List.generate_kwargs(
                "eu", query=query, prop=prop, protocol=protocol, namespace=namespace
            )
        )
    ...
    def checkuserlog(
        self, user=None, target=None, limit=10, dir="older", start=None, end=None
    ):
        """Retrieve checkuserlog items as a generator."""

        kwargs = dict(
            listing.List.generate_kwargs(
                "cul", target=target, start=start, end=end, dir=dir, user=user
            )
        )
        return listing.NestedList(
            "entries", self, "checkuserlog", "cul", limit=limit, **kwargs
        )
    ...
    def search(self, search, namespace="0", what=None, redirects=False, limit=None):
        ...
        Returns:
            aiomwclient.listings.List: Search results iterator
        """
        kwargs = dict(
            listing.List.generate_kwargs(
                "sr", search=search, namespace=namespace, what=what
            )
        )
        if redirects:
    ...
    def usercontributions(
        ...
        return listing.List(
            self, "usercontribs", "uc", limit=limit, uselang=uselang, **kwargs
        )
    ...
    async def expandtemplates(self, text, title=None, generatexml=False):
        """
        ...
        result = await self.get("expandtemplates", text=text, **kwargs)
    ...
    async def ask(self, query, title=None):
        """
        ...
        while offset is not None:
            results = await self.raw_api(
                "ask",
                query=u"{query}|offset={offset}".format(query=query, offset=offset),
                http_method="GET",
                **kwargs
            )
            await self.handle_api_result(results)  # raises APIError on error
